...
FITR_FMT  = "ped{ped}_led{led}_board{board}"
...
def main():
    ...
    for r in rows:
        ...
        for board in range(NBOARDS):
            ...
            base_fitr = FITR_FMT.format(ped=ped, led=led, board=board)
            ...
            # --- chi2 / ndf ---
            chi2 = float(fitr.Chi2()) if fitr else float("nan")
            ndf  = int(fitr.Ndf()) if fitr else -1
