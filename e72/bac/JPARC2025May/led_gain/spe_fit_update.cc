...
TF1* FitSPE(TH1* hOn, TH1* hOff, TDirectory* outDir, const char* key){
  ...
  f->SetParLimits(4, 0.9*Q1_init, 1.1*Q1_init);
...
void spe_fit_update(int pednumber, int runnumber, int board){
  ...
  TFile *fout = new TFile(Form("spe_result_update/spe_result_ped%d_led%d_board%d.root",pednumber,runnumber,board), "RECREATE");
