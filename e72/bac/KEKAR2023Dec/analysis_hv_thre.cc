...
struct TurnOnFitResult {
  int status = -999;   // 0이면 대체로 OK
  double plateau = 0.0, plateau_err = 0.0;
  double x50 = 0.0,     x50_err     = 0.0;
  double width = 0.0,   width_err   = 0.0;
  double fit_xmin = 0.0, fit_xmax = 0.0;
};

// =====================
// Turn-on (step-like) 함수: erf
// f(x) = 0.5*plateau*(1 + erf((x-x50)/(sqrt(2)*width)))
// =====================
double TurnOnErf(double* x, double* p)
{
  const double xx = x[0];
  const double plateau = p[0];
  const double x50 = p[1];
  const double w = p[2];
  if (w <= 0) return 0;
  return 0.5 * plateau * (1.0 + TMath::Erf((xx - x50) / (TMath::Sqrt2() * w)));
}
...
//   - h_full: denom
//   - h_cut : numer
//   - name_prefix: 객체 이름 prefix
//   - out_png: ""이면 저장 안 함
//   - y_lo/y_hi: turn-on 구간 자동 찾는 기준
// =====================
TurnOnFitResult FitEffTurnOn(TH1* h_full,
                             TH1* h_cut,
                             const char* name_prefix = "baca",
                             const char* out_png = "",
                             double y_lo = 0.00,
                             double y_hi = 1.0)
{
  ...
  if (!h_full || !h_cut) {
    std::cerr << "[FitEffTurnOn] ERROR: null histogram\n";
    return out;
  }
  if (h_full->GetNbinsX() != h_cut->GetNbinsX()) {
    std::cerr << "[FitEffTurnOn] ERROR: binning mismatch\n";
    return out;
  }

  // efficiency graph
  auto* geff = new TGraphAsymmErrors();
  geff->SetName(Form("%s_geff", name_prefix));
  geff->SetTitle("Efficiency;X;#varepsilon");

  // Clopper-Pearson 68.3% binomial CI
  geff->Divide(h_cut, h_full, "cl=0.683 b(1,1) mode");

  const double xmin = h_full->GetXaxis()->GetXmin();
  const double xmax = h_full->GetXaxis()->GetXmax();

  // turn-on 구간 자동 추정 (eff가 y_lo 넘는 첫 점 ~ y_hi 넘는 첫 점)
  double x_lo = xmin, x_hi = xmax;
  bool found_lo = false, found_hi = false;

  const int n = geff->GetN();
  for (int i = 0; i < n; ++i) {
    double xx, yy;
    geff->GetPoint(i, xx, yy);
    if (!found_lo && yy >= y_lo) { x_lo = xx; found_lo = true; }
    if (!found_hi && yy >= y_hi) { x_hi = xx; found_hi = true; }
  }

  if (!found_lo) x_lo = xmin;
  if (!found_hi) x_hi = xmax;
  if (x_hi < x_lo) std::swap(x_lo, x_hi);

  // 너무 좁으면 전체 범위
  if (x_hi - x_lo < 1e-9) { x_lo = xmin; x_hi = xmax; }

  // 약간 pad
  const double pad = 0.10 * (x_hi - x_lo);
  x_lo = std::max(xmin, x_lo - pad);
  x_hi = std::min(xmax, x_hi + pad);

  out.fit_xmin = x_lo;
  out.fit_xmax = x_hi;

  // fit func
  auto* fturn = new TF1(Form("%s_turnon", name_prefix), TurnOnErf, x_lo, x_hi, 3);
  fturn->SetParNames("plateau", "x50", "width");
  fturn->SetParameters(1.0, 0.5*(x_lo+x_hi), std::max(1e-3, (x_hi-x_lo)/10.0));
  fturn->SetParLimits(0, 0.0, 1.2);
  fturn->SetParLimits(2, 1e-6, (x_hi-x_lo));

  // Fit: R(range), S(store), Q(quiet)
  auto fitptr = geff->Fit(fturn, "RSQ");
  out.status = fitptr;

  out.plateau = fturn->GetParameter(0);
  out.x50     = fturn->GetParameter(1);
  out.width   = fturn->GetParameter(2);

  out.plateau_err = fturn->GetParError(0);
  out.x50_err     = fturn->GetParError(1);
  out.width_err   = fturn->GetParError(2);

  // optional draw
  if (out_png && std::string(out_png).size() > 0) {
    auto* c = new TCanvas(Form("%s_c", name_prefix), "eff", 900, 700);
    c->Divide(1,2);

    c->cd(1);
    gPad->SetGrid();
    h_full->SetLineColor(kBlue+1);
    h_full->SetLineWidth(2);
    h_cut->SetLineColor(kRed+1);
    h_cut->SetLineWidth(2);
    h_full->SetTitle("Numerator/Denominator;X;Entries");
    h_full->Draw("HIST");
    h_cut->Draw("HIST SAME");

    auto* leg = new TLegend(0.65,0.75,0.88,0.88);
    leg->AddEntry(h_full, "full (denom)", "l");
    leg->AddEntry(h_cut,  "cut (num)",   "l");
    leg->Draw();
...
int main(int argc, char* argv[]){
 ...
 int pedrun, runnumber, x_pos, y_pos, bac_hv, bac_thre, BAC_thick, KVC_thick, kvc_hv, kvc_thre;
  ...
  auto r = FitEffTurnOn(hist_baca_full, hist_baca_cut, "baca", Form("picture/run00%d_thre_cut.pdf",runnumber));
  

  
  TFile *output = new TFile("kek_result_hv_thre.root","UPDATE");
