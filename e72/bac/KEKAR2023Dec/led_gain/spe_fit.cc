...
void spe_fit(int board, int channel){
  
  //TFile *fin = TFile::Open("BAC_LED_57V.root", "READ");
  TFile *fin = TFile::Open("BAC_LED_KEK.root", "READ");
  ...
  //TFile *fout = new TFile(Form("spe_result/spe_result%d_%d.root",board,channel), "RECREATE");
  TFile *fout = new TFile(Form("spe_result/spe_result%d_hv%d.root",board,channel+56), "RECREATE");
  //const char* key = Form("mppc%d_%d", board, channel);
  const char* key = Form("mppc%d_hv%d", board, channel+56);
