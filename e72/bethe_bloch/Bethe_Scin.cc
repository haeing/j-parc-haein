...
double Caldedx(double mass, double beta){
  Double_t rho=1.023; //[g cm-3] density of the material (EJ-232)
  Double_t num_C = 9.0;
  Double_t num_H = 12.0;

  Double_t ratio_C = num_C/(num_C + num_H);
  Double_t ratio_H = num_H/(num_C + num_H);
  Double_t ZoverA = (6.0*num_C + 1.0*num_H)/ (12.0*num_C + 2.0*num_H); //[mol g-1]

  
  Double_t I = ratio_C *13.0 * 6.0 + ratio_H*19.0*1.0; //[eV] Mean excitation energy : 10 eV * Z
...
void Bethe_Scin(){
  ...
  for(int i=0;i<n;i++){
    ...
    gr_p->AddPoint(mom,Caldedx(mp,beta_p)*0.5*keVToMeV);
    gr_k->AddPoint(mom,Caldedx(mk,beta_k)*0.5*keVToMeV);
    gr_pi->AddPoint(mom,Caldedx(mpi,beta_pi)*0.5*keVToMeV);
    gr_e->AddPoint(mom,Caldedx(me,beta_e)*0.5*keVToMeV);
  }
  ...
  //gPad->SetLogx();
  gPad->SetLogy();
  mg->SetTitle("Bethe-bloch formula;p [MeV/c];Energy loss inside the Scintillator [MeV/5 mm]");
